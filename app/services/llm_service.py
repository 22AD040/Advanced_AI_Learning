...
Enhanced version with Groq API for content generation
"""
...
class LLMService:
    """Service for handling LLM interactions with Groq API"""
    ...
    def __init__(self):
        ...
        try:
            ...
            if api_key and api_key.strip():
                ...
                print(f"✅ Groq initialized successfully with API key: {masked_key}")
                print(f"📌 Available models: llama-3.3-70b-versatile, mixtral-8x7b-32768, gemma2-9b-it")
            else:
                print("❌ No valid Groq API key found - using ENHANCED fallback content")
        except Exception as e:
            print(f"❌ Groq initialization error: {e} - using ENHANCED fallback content")
            self.groq_available = False
    
    def _call_groq_api(self, prompt: str, model: str = "llama-3.3-70b-versatile", temperature: float = 0.7) -> Optional[str]:
        """Helper method to call Groq API with consistent error handling"""
        ...
        try:
            chat_completion = self.groq_client.chat.completions.create(
                messages=[
                    {
                        "role": "system",
                        "content": "You are an EXPERT EDUCATOR and academic content creator. You specialize in creating comprehensive, detailed, and accurate educational materials. Always respond with valid JSON when requested. Include specific examples, real-world applications, and detailed explanations."
                    },
                    {
                        "role": "user",
                        "content": prompt,
    ...
    def generate_study_content_with_ai(self, topic: str, level: str) -> Dict:
        ...
        try:
            print(f"🚀 Generating content for: {topic} at {level} level using Groq API")
...
Generate the JSON now:"""
            ...
            if not response_text:
                print("⚠️ No response from Groq API, using fallback")
                return self._get_advanced_fallback_content(topic, level)
            
            print(f"📝 Received response from Groq ({len(response_text)} chars)")
    ...
    def generate_quiz_with_ai(self, topic: str, num_questions: int = 10) -> Dict:
        ...
        try:
            print(f"📝 Generating {num_questions} quiz questions for {topic} using Groq")
...
Make each question TEACH something valuable even if answered incorrectly!"""
            ...
            response_text = self._call_groq_api(prompt, temperature=0.8)
            
            if not response_text:
                return self._get_advanced_fallback_quiz(topic, num_questions)
            
            quiz_data = self._extract_json(response_text)
            
            if quiz_data and 'questions' in quiz_data:
                print(f"✅ Successfully generated {len(quiz_data['questions'])} quiz questions")
                return quiz_data
            else:
                return self._get_advanced_fallback_quiz(topic, num_questions)
    ...
    def generate_mindmap_with_ai(self, topic: str) -> Dict:
        ...
        try:
            print(f"🗺️ Generating mindmap for {topic} using Groq")
...
Make it comprehensive and actionable for learners!"""
            ...
            response_text = self._call_groq_api(prompt, temperature=0.6)
            
            if not response_text:
                return self._get_advanced_fallback_mindmap(topic)
            
            mindmap = self._extract_json(response_text)
            
            if mindmap:
                print(f"✅ Successfully generated mindmap for {topic}")
                return mindmap
            else:
                return self._get_advanced_fallback_mindmap(topic)
...
"""
    ...
    def generate_response(self, query: str, role: str, context: Dict = None) -> str:
        """Generate response based on user role using Groq API"""
        if self.groq_available:
            try:
                ...
                response_text = self._call_groq_api(prompt, temperature=0.7)
                if response_text:
